...
def resolve_data_per_day_path(study, sensor):
    """
    Resolve the per-sensor analytics CSV path.

    Some studies store `Data_per_day_<sensor>_latest.csv` directly under the
    study folder, while others place it inside `outputs/`. This helper checks
    both locations and returns the first existing match.
    """
    candidates = data_per_day_candidates(study, sensor)
...
def default_callbacks(app):
    ...
    def populate_date_filter(study, sensor):
        ...
        path = resolve_data_per_day_path(study, sensor)
    ...
    def update_heat_map(study, sensor, date_filter, subject_filter):
        ...
        def build_plot_title(sensor_name, use_month_axis, selected_subject_values):
            sensor_label = prettify_label(sensor_name)
            time_label = "Month" if use_month_axis else "Study Day"
            if selected_subject_values:
                subject_count = len(selected_subject_values)
                if subject_count == 1:
                    subject_label = f"Selected Subject ({selected_subject_values[0]})"
                else:
                    subject_label = f"{subject_count} Selected Subjects"
            else:
                subject_label = "All Subjects"
            return f"{sensor_label}: Daily Totals by {subject_label} Across {time_label}"
        ...
        if subject_filter:
            selected_subjects = [str(v) for v in (subject_filter if isinstance(subject_filter, list) else [subject_filter])]
            df = df[df[subj_col].isin(selected_subjects)]
        ...
        fig.update_layout(
            title=build_plot_title(sensor, x_is_date, selected_subjects),
    ...
    def update_data_table(study, sensor, date_filter, subject_filter):
        if not (study and sensor):
            raise PreventUpdate
        ...
        if subject_filter and subj_col:
            selected_subjects = [str(v) for v in (subject_filter if isinstance(subject_filter, list) else [subject_filter])]
            df = df[df[subj_col].isin(selected_subjects)]

        if date_filter == "Date" and "Date" in df.columns:
            df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
            df = df.sort_values(by=["Date", subj_col] if subj_col else ["Date"])
            df["Date"] = df["Date"].dt.strftime("%Y-%m-%d")
        elif date_filter == "Study_day" and "Study_day" in df.columns:
            df["Study_day"] = pd.to_numeric(df["Study_day"], errors="coerce")
            sort_columns = ["Study_day"]
            if subj_col:
                sort_columns.append(subj_col)
