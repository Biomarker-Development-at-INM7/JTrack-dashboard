...
from . import views
from jdash.analytics import dash_plot
from jdash.config import constants as constants


urlpatterns = [
    path('favicon.ico', RedirectView.as_view(url=static_url("icons/favicon.ico"), permanent=True)),
    ...
    path('password-reset/', views.password_reset_request, name=constants.url_name_for_password_reset),
    path('password-reset/done/', views.password_reset_done, name=constants.url_name_for_password_reset_done),
    path('reset/<uidb64>/<token>/', views.password_reset_confirm, name=constants.url_name_for_password_reset_confirm),
    path('reset/done/', views.password_reset_complete, name=constants.url_name_for_password_reset_complete),
